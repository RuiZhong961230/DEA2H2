...
def DEcur2mbest(Off, i):
    global DimSize, Pop, FitPop, PopSize, BestIndi
    Xmean = np.mean(Pop, axis=0)
    Off[i] = Pop[i] + np.random.rand() * (Xmean - Pop[i]) + np.random.rand() * (BestIndi - Pop[i])
    Off[i] = np.clip(Off[i], LB, UB)
...
def main():
    ...
    for i in range(len(Probs)):
        ...
        for j in range(Trials):
            ...
            Trace = DEAH(Probs[i].evaluate)
            All_Trial_Best.append(Trace)
        np.savetxt("./DEAH_Data/Engineer/" + Names[i] + ".csv", All_Trial_Best, delimiter=",")


if __name__ == "__main__":
    if os.path.exists('DEAH_Data/Engineer') == False:
        os.makedirs('DEAH_Data/Engineer')
